<?php /* STYLE/DEFAULT $Id: overrides.php 6038 2010-10-03 ajdonnison $ */
...
class CTabBox extends CTabBox_core {
	...
	function show($extra='', $js_tabs = false) {
		...
		$uistyle = ($AppUI->getPref('UISTYLE') ? $AppUI->getPref('UISTYLE') 
		...
		if (@$AppUI->getPref('TABVIEW') == 0) {
			...
			if ($extra) {
				echo ('<table border="0" cellpadding="2" cellspacing="0" width="100%">' 
				      . "\n<tr>\n" . $extra . "</tr>\n</table>\n");
			} else { echo '<img src="./images/shim.gif" height="10" width="1" alt="" />' . "\n"; }
		}
		if ($this->active < 0 || @$AppUI->getPref('TABVIEW') == 2) {
			...
			foreach ($this->tabs as $k => $v) {
				...
				if ($this->javascript) { $s .= "javascript:" . $this->javascript . "({$this->active}, $k)";	} 
				  else if ($js_tabs) {	$s .= 'javascript:show_tab(' . $k . ')'; } 
					else {	$s .= $this->baseHRef.'tab='.$k; }
				
				$s .='">'.(($v[2]) ? $v[1] : $AppUI->_($v[1])).'</a>&nbsp;</td>' . "\n";
				$s .= ('<td valign="middle"><img id="righttab_' 
						. $k . '" src="./style/' 
						. $uistyle . '/images/tab' 
			...
			if ($this->tabs[$this->active][0] != "") {
				...
				if (!$js_tabs) require $this->baseInc.$this->tabs[$this->active][0].'.php';
			}
			if ($js_tabs) {
				foreach ($this->tabs as $k => $v) { 
