...
if (!(isset($showEditCheckbox))) { $showEditCheckbox = $dPconfig['direct_edit_assignment']; }

/*  tasks.php
 This file contains common task list rendering code used by modules/tasks/index.php and modules/projects/vw_tasks.php
...
if (empty($query_string)) { $query_string = "?m=$m&amp;a=$a"; }
$cols = 13; // Number of columns (used to calculate how many columns to span things through)
...
if (isset($_GET['pin'])) {
	...
	if (!db_exec($sql)) { $AppUI->setMsg('ins/del err', UI_MSG_ERROR, true); }
	$AppUI->redirect('', -1);
}

if ($task_id > 0) { $_GET['open_task_id'] = $task_id; }
...
if (isset($_POST['show_task_options'])) { $AppUI->setState('TaskListShowIncomplete', (int)dPgetParam($_POST, 'show_incomplete', 0)); }
$showIncomplete = $AppUI->getState('TaskListShowIncomplete', 0);

$project = new CProject;
$allowedProjects = $project->getAllowedSQL($AppUI->user_id);
if (count($allowedProjects)) { $where_list = implode(' AND ', $allowedProjects);}
...
$q->addJoin('tasks', 't1', 'prj.project_id = t1.task_project');
if ($where_list) { $q->addWhere($where_list);}
$q->addGroup('project_id');
$psql2 = $q->prepare();
$q->clear();
...
if ($canAccessTask) {
	$prc = db_exec($psql); echo db_error();
	while ($row = db_fetch_assoc($prc)) { $projects[$row['project_id']] = $row;	}
	$prc2 = db_exec($psql2); echo db_error();
	while ($row2 = db_fetch_assoc($prc2)) { if ($projects[$row2['project_id']]) { array_push($projects[$row2['project_id']], $row2); } }
}
...
if ($pinned_only) { $where .= ' AND task_pinned = 1 '; }

$f = (($f) ? $f : '');
$never_show_with_dots = array(/*'children', */''); //used when displaying tasks
switch ($f) {
	case 'all': break;
	case 'myfinished7days': $where .= ' AND ut.user_id = '.$user_id;
	...
	case 'myproj': $where .= ' AND project_owner = ' . $user_id;	break;
	case 'mycomp':
		if (!($AppUI->user_company)) {	$AppUI->user_company = 0; }
	...
	case 'unassigned':
		...
		break;
	case 'taskcreated': $where .= ' AND tsk.task_owner = ' . $user_id;	break;
	default:
		$from = $dbprefix.'user_tasks as ut5, '.$from;
		$where .= (' AND tsk.task_project = prj.project_id AND ut5.user_id = ' . $user_id . ' AND ut5.task_id = tsk.task_id');
		break;
}

if ($project_id && $showIncomplete) { $where .= ' AND (task_percent_complete < 100 or task_percent_complete is null)'; }

$task_status = 0;
if ($min_view && isset($_GET['task_status'])) {	$task_status = intval(dPgetParam($_GET, 'task_status', null)); } 
else if (!($currentTabName)) { $task_status = intval($AppUI->getState('inactive')); } // If we aren't tabbed we are in the tasks list.
else if (mb_stristr($currentTabName, 'inactive')) { $task_status = '-1'; }
...
if (count($allowedTasks)) {	$where .= ' AND ' . implode(' AND ', $allowedTasks); }
$allowedChildrenTasks = $obj->getAllowedSQL($AppUI->user_id, 'task_parent');
if (count($allowedChildrenTasks)) { $where .= ' AND ' . implode(' AND ', $allowedChildrenTasks);}
// echo "<pre>$where</pre>";
...
if ($canAccessTask) { $ptrc = db_exec($tsql); if ($ptrc != false) { $nums = db_num_rows($ptrc); } echo db_error(); } 
else { 	$nums = 0; }
...
for ($x=0; $x < $nums; $x++) {
	...
	echo db_error();
	for ($xx=0; $xx < $nnums; $xx++) { $row['task_assigned_users'][] = db_fetch_assoc($paurc);	}
	//pull the final task row into array
	$projects[$row['task_project']]['tasks'][] = $row;
	$all_tasks[$row['task_id']] = $row['task_id'];
}

if ($open_task_all)			{ $tasks_opened = $all_tasks;}
 else if ($close_task_all)	{ $tasks_opened = array();}
 else if ($open_one_level)	{ openCloseOneLevel($projects, true);}
 else if ($close_one_level) { openCloseOneLevel($projects, false);}
 else if ($open_task_id)	{ openClosedTask($open_task_id);}
 else if ($close_task_id)	{ closeOpenedTask($close_task_id);}
?>
...
function chAssignment(project_id, rmUser, del) {
	...
	for (var i=0;i< f.elements.length;i++) {
		var e = f.elements[i];
		// only if it's a checkbox.
		if (e.type == "checkbox" && e.checked == true && e.name != 'master') { c++; f.htasks.value = f.htasks.value +", "+ e.value;	}
	}
...
<table width="100%" border="0" cellpadding="2" cellspacing="1" class="tbl">
	...
	<tr>
		<td>
			...
			?>" width="16" height="16" border="0" alt="<?php  echo (($project_id) ? $AppUI->_('show other projects') : $AppUI->_('show only this project')); ?>" />
  			</a>
  		</td>
		...
		<td colspan="3" align="right" valign="middle">
			<table width="100%" border="0">
				<tr>
					<td align="right">
					<?php  $hasEditableTask = ((isset($canEdit) && $canEdit) ? true : false);
...
<?php
		...
		if (is_array($p['tasks'])) {
			foreach ($p['tasks'] as $i => $t) {
				...
				if ($t['task_parent'] != $t['task_id']) { array_push($children_of[$t['task_parent']], $t['task_id']); }
			}
		}
		...
		if (is_array($p['tasks'])) {
			...
			foreach ($p['tasks'] as $i => $t1) {
				// Record summaries
				if (!$t1['task_dynamic']) { $summaries['duration'] += $t1['task_duration'];	}
				if ($t1['task_start_date'] < $summaries['start_date']) { $summaries['start_date'] = $t1['task_start_date']; }
				if ($t1['task_end_date'] > $summaries['end_date']) {	$summaries['end_date'] = $t1['task_end_date'];	}
				if ($task_sort_item1) { showtask($t1, -1, true, false, true); } // already user sorted so there is no call for a "task tree" or "open/close" links
				else {
					if ($t1['task_parent'] == $t1['task_id']) {
						$is_opened = !empty($tasks_opened[$t1['task_id']]);	
						$no_children = empty($children_of[$t1['task_id']]); //check for child
						showtask($t1, 0, $is_opened, false, $no_children);
						if ($is_opened && !($no_children)) { findchild($p['tasks'], $t1['task_id']); }
...
<table>
	<tr>
		...
		<td>=<?php echo $AppUI->_('Done'); ?>&nbsp;&nbsp;
			...
			<?php echo $AppUI->_('Expand'); ?>&nbsp;
			<a href="<?php echo 'index.php'.$query_string.'&amp;open_task_all=1'; ?>"><?php echo $AppUI->_('All'); ?></a>/
			<a href="<?php echo 'index.php'.$query_string.'&amp;open_one_level=1'; ?>"><?php echo $AppUI->_('One Level'); ?></a> : 

			<?php echo $AppUI->_('Collapse'); ?>&nbsp;
			<a href="<?php echo 'index.php'.$query_string.'&amp;close_task_all=1';?>"><?php echo $AppUI->_('All'); ?></a>/
			<a href="<?php echo 'index.php'.$query_string.'&amp;close_one_level=1';?>"><?php echo $AppUI->_('One Level'); ?></a>
