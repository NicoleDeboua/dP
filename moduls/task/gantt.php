...
if ($caller == 'todo') {
 	...
 	if (!$showArcProjs) { $q->addWhere('project_status <> 7');	}
	...
	if ($f != 'myinact') { $q->addWhere('task_status > -1'); }
	switch ($f) {
		case 'all': break;
		case 'myproj': $q->addWhere('project_owner = ' . $AppUI->user_id); break;
		case 'mycomp': $q->addWhere('project_company = ' . $AppUI->user_company); break;
		case 'myinact': 
...
foreach ($proTasks_data as $data_row) {
	...
	if ($task->canAccess($AppUI->user_id)) { $proTasks[] = $data_row; }
}
...
foreach ($proTasks as $row) {
	...
	if ($row['task_end_date'] == '0000-00-00 00:00:00') {
		...
		} else { $row['task_end_date'] = $p_end_date; }
	}
	...
	if ($tsd->before(new CDate($start_min))) { $start_min = $row['task_start_date']; }
...
if ($day_diff > 240) { $graph->ShowHeaders(GANTT_HYEAR | GANTT_HMONTH); } //more than 240 days 
else if ($day_diff > 90) {
...
foreach ($projects as $p) {
	...
	for ($i=0; $i < $tnums; $i++) {
		$t = $p['tasks'][$i];
		if (!(isset($parents[$t['task_parent']]))) { $parents[$t['task_parent']] = false; }
		if ($t['task_parent'] == $t['task_id']) {	showgtask($t);	findgchild($p['tasks'], $t['task_id']);	}
	}
	// Check for ophans.
	foreach ($parents as $id => $ok) { if (!($ok)) { findgchild($p['tasks'], $id); } }
}
...
for ($i = 0; $i < count(@$gantt_arr); $i ++) {
	...
	if (!$end) { $end = $start; $cap .= ' (no end date)'; }
	...
	if ($showLabels == '1') {
		...
		foreach ($res as $rw) {
			switch ($rw['perc_assignment']) {
				case 100: 	$caption .= ($rw['user_username'] . ';');	break;
				default:	$caption .= ($rw['user_username'] . '[' . $rw['perc_assignment'] . '%];');	break;
			}
		}
		$q->clear();
