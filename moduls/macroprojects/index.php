<?php  /* MACROPROJECTS index.php, v 0.1.0 2012-05-30 */
...
// End of macroproject status update retrieve any state parameters
if (isset($_GET['tab'])) { $AppUI->setState('MacroProjIdxTab', intval(dPgetCleanParam($_GET, 'tab'))); }
$tab = $AppUI->getState('MacroProjIdxTab') !== NULL ? $AppUI->getState('MacroProjIdxTab') : 500;
$currentTabId = $tab;
$active = intval(!$AppUI->getState('MacroProjIdxTab'));
if (isset($_POST['company_id'])) { $AppUI->setState('MacroProjIdxCompany', intval($_POST['company_id'])); }
$company_id = (($AppUI->getState('MacroProjIdxCompany') !== NULL) ? $AppUI->getState('MacroProjIdxCompany') : $AppUI->user_company);
$company_prefix = 'company_';

if (isset($_POST['department'])) {	$AppUI->setState('MacroProjIdxDepartment', $_POST['department']);
	unset($company_id); //if department is set, ignore the company_id field
}
$department = (($AppUI->getState('MacroProjIdxDepartment') !== NULL) ? $AppUI->getState('MacroProjIdxDepartment') : ($company_prefix . $AppUI->user_company));
...
$macroorderby = (($AppUI->getState('MacroProjIdxOrderBy')) ? $AppUI->getState('MacroProjIdxOrderBy') : 'macroproject_end_date');
$AppUI->setState('MacroProjIdxOrderDir', $orderdir);

// prepare the users filter
if (isset($_GET['show_owner'])) { $AppUI->setState('MacroProjIdxowner', intval($_GET['show_owner'])); }
  else if (isset($_POST['show_owner'])) { $AppUI->setState('MacroProjIdxowner', intval($_POST['show_owner'])); }
...
$bufferUser = arraySelect($userRows, 'show_owner','class="text" onchange="javascript:document.pickUser.submit()""', $owner);
...
/* setting this to filter macroproject_list_data function below
 ...
 5 = completed
 7 = archived
Because these are "magic" numbers, if the values for macroprojectStatus change under 'System Admin', they'll need to change here as well (sadly).
*/
if ($tab != 7 && $tab != 8) { $macroproject_status = $tab; } else if ($tab == 0) { $macroproject_status = 0; }
if ($tab == 5 || $tab == 7) { $macroproject_active = 0; }

//for getting permissions for records related to macroprojects
$obj_macroproject = new CMacroProject();

macroprojects_list_data(); // collect the full (or filtered) macroprojects list data via function in macroprojects.class.php
$mptype =  dPgetSysVal('MacroProjectType'); // Get Type 
$mpstatus =  dPgetSysVal('MacroProjectStatus'); // Get Status

// setup the title block
$titleBlock = new CTitleBlock('Macroprojects', 'macroprojects.gif', $m, ($m . '.' . $a));
...
$titleBlock->addCell();
if ($canAuthor) { $titleBlock->addCell(('<form action="?m=macroprojects&amp;a=addedit" method="post">' . "\n" 
...
if ($owner > 0) { $q->addWhere('p.macroproject_owner = ' . $owner); }
if (isset($department)) {
	$q->addJoin('macroproject_departments', 'pd', 'pd.macroproject_id = p.macroproject_id');
	if (!$addPwOiD) { $q->addWhere('pd.department_id in (' . implode(',',$dept_ids) . ')'); }
} else if ($company_id &&!$addPwOiD) { $q->addWhere('p.macroproject_company = ' . $company_id); }
...
foreach ($statuses as $k => $v) {
	$macroproject_status_tabs[$v['macroproject_status']] = ($AppUI->_($macroproject_types[$v['macroproject_status']]). ' (' . $v['count'] . ')');
	$all_macroprojects += $v['count']; //count all macroprojects
}
...
/** Now, we will figure out which vw_idx file are available for each macroproject status using the $fixed_status array */
...
foreach ($macroproject_types as $status_id => $status_title) {
	...
	$macroproject_status_file[$status_id] = ((isset($fixed_status[$status_title])) ? $fixed_status[$status_title] : 'vw_idx_proposed');
}
...
foreach ($macroproject_types as $psk => $macroproject_status) {
		$tabBox->add($macroproject_status_file[$psk], (($macroproject_status_tabs[$psk]) ? $macroproject_status_tabs[$psk] : $AppUI->_($macroproject_status)), true, $psk);
}
$min_view = true;
if (getPermission('finances', 'access')) { $tabBox->add('vw_finances', 'Finances'); }
if (getPermission('forums', 'access')) 	{ $tabBox->add('vw_forums', 'Forums'); }	// Carefull : forums are useless and bug !
if (getPermission('files', 'access'))  	{ $tabBox->add('vw_files', 'Files'); }	// Carefull : forums are useless and bug !
$tabBox->add('vw_gantt', 'Gantt');
$tabBox->show();
?>
