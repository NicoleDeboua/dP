<?php /* MACRO_PROJECTS gantt.php, v 0.1.0 2012/05/30 */
...
global $macroproFilter, $macroprojectStatus, $showInactive, $showLabels, $showAllGantt;
...
$macroproFilter = dPgetParam($_REQUEST, 'macroproFilter', '-1');
...
$macroprojectStatus = arrayMerge(array('-2' => $AppUI->_('All w/o in progress'), 
                                  '-3' => $AppUI->_(($AppUI->user_id == $user_id) 
                                  ? 'My macroprojects' : "User's macroprojects")), $macroprojectStatus);

$pjobj = new CMacroProject;
...
// pull valid macroprojects and their percent complete information
// GJB: Note that we have to special case duration type 24 and this refers to the hours in a day, NOT 24 hours
$q->addTable('macroprojects', 'mp');
$q->addQuery('DISTINCT mp.macroproject_id, macroproject_color_identifier, macroproject_name, macroproject_start_date' 
             . ', macroproject_end_date'//, max(t1.task_end_date) AS macroproject_actual_end_date' 
...
if ($department > 0) {
	...
	if (!$addPwOiD) {	$q->addWhere('mpd.department_id = ' . $department);	} 
	 else {
		// Show macroprojects where the macroproject Owner is in the given department
		$q->addWhere('mp.macroproject_owner IN ('. ((!empty($owner_ids)) ? implode(',', $owner_ids) : 0) . ')');
	}
} else if ($company_id != 0 && !$addPwOiD) { $q->addWhere('macroproject_company = ' . $company_id);}

if ($macroproFilter == '-4') {	$q->addWhere('macroproject_status != 7');} 
  else if ($macroproFilter == '-3') {$q->addWhere('macroproject_owner = ' . $user_id);} 
  else if ($macroproFilter == '-2') {$q->addWhere('macroproject_status != 3');} 
  else if ($macroproFilter != '-1') {$q->addWhere('macroproject_status = ' . $macroproFilter);}

if ($user_id && $m_orig == 'admin' && $a_orig == 'viewuser') {	$q->addWhere('macroproject_owner = ' . $user_id);}

if ($showInactive != '1') {	$q->addWhere('macroproject_status != 7');}
$pjobj->setAllowedSQL($AppUI->user_id, $q, null, 'mp');
$q->addGroup('mp.macroproject_id');
$q->addOrder('macroproject_name');//, task_end_date DESC');
...
$graph->scale->actinfo->SetColTitles(array($AppUI->_('MacroProject name', UI_OUTPUT_RAW), 
                                           $AppUI->_('Start Date', UI_OUTPUT_RAW), 
                                           $AppUI->_('Finish', UI_OUTPUT_RAW), 
                                           $AppUI->_('Actual End', UI_OUTPUT_RAW)), 
                                     array(160, 70, 70, 70));

$tableTitle = (($macroproFilter == '-1') ? $AppUI->_('All MacroProjects') : $macroprojectStatus[$macroproFilter]);
...
if ($start_date && $end_date) {
	...
	$d_start = new CDate();
	$d_end = new CDate();
	for ($i = 0, $xi = count(@$macroprojects); $i < $xi; $i++) {
		$start = mb_substr($mp['macroproject_start_date'], 0, 10);
		$end = mb_substr($mp['macroproject_end_date'], 0, 10);
...
if (!is_array($macroprojects) || sizeof($macroprojects) == 0) {
	$d = new CDate();
...
if (is_array($macroprojects)) {
	foreach ($macroprojects as $mp) {
		...
		$start = (($mp['macroproject_start_date'] > '0000-00-00 00:00:00') ? $mp['macroproject_start_date'] 
		          : date('Y-m-d H:i:s'));
		$end_date = (($mp['macroproject_end_date'] > '0000-00-00 00:00:00') ? $mp['macroproject_end_date'] 
		...
		$progress = $mp['macroproject_percent_complete'] + 0;
        ...
        if ($showLabels) {
			$caption .= ($AppUI->_($macroprojectStatus[$mp['macroproject_status']]) . ', ');
			$caption .= (($mp['macroproject_status'] <> 7) ? $AppUI->_('active') : $AppUI->_('archived'));
        }
		$enddate = new CDate($end);
		$startdate = new CDate($start);
		$actual_end = (($mp['macroproject_actual_end_date']) ? $mp['macroproject_actual_end_date'] : $end);
        ...
        if ($mp['macroproject_status'] != '3' || $mp['macroproject_status'] == '7') {
		...
		if ($showAllGantt) {
			...
			// select for tasks for each macroproject	
			$q->addTable('tasks', 't');
			$q->addQuery('DISTINCT t.task_id, t.task_name, t.task_start_date, t.task_end_date' 
			             . ', t.task_milestone, t.task_dynamic');
			$q->addJoin('macroprojects', 'mp', makeWhereClauseEachProjectOfAMacroProject('mp.macroproject_id', 't.task_project = '));
			$q->addWhere('mp.macroproject_id = '. $mp['macroproject_id']);
			...
			foreach ($tasks as $t) {
				...
				if ($t['task_milestone'] != 1) {
					...
					$bar2->title->SetColor(bestColor('#ffffff', 
					                                 ('#' . $mp['macroproject_color_identifier']), 
					                                 '#000000'));
					$bar2->title->SetFont(FF_CUSTOM, FS_NORMAL, 9);
					$bar2->SetFillColor(('#' . $mp['macroproject_color_identifier']));
					$graph->Add($bar2);
				} else {
				...
				foreach ($workers as $w) {	
					...
					$bar3->title->SetColor(bestColor('#ffffff', 
					                                 ('#' . $mp['macroproject_color_identifier']), 
					                                 '#000000'));
					$bar3->SetFillColor('#' . $mp['macroproject_color_identifier']);		
					$graph->Add($bar3);
				}
...
} // End of check for valid macroprojects array.*/
unset($macroprojects);
