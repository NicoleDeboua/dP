...
if (isset($_GET['macroorderby']) && in_array($_GET['macroorderby'], $valid_ordering)) {
    $orderdir = ($AppUI->getState('UsrMacroProjIdxOrderDir') ? ($AppUI->getState('UsrMacroProjIdxOrderDir')== 'asc' ? 'desc' : 'asc') : 'desc');    
    $AppUI->setState('UsrMacroProjIdxOrderBy', $_GET['macroorderby']);
    $AppUI->setState('UsrMacroProjIdxOrderDir', $orderdir);
}
$macroorderby  = ($AppUI->getState('UsrMacroProjIdxOrderBy') ? $AppUI->getState('UsrMacroProjIdxOrderBy') : 'macroproject_end_date');
$orderdir = ($AppUI->getState('UsrMacroProjIdxOrderDir') ? $AppUI->getState('UsrMacroProjIdxOrderDir') : 'asc');
...
<tr>
	...
	<th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=macroproject_name" class="hdr"> <?php echo $AppUI->_('MacroProject Name'); ?>
		</a>
	</th>
          <th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=macroproject_start_date" class="hdr"> <?php echo $AppUI->_('Start'); ?>
		</a>
	</th>
	<th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=macroproject_duration" class="hdr">	<?php echo $AppUI->_('Duration'); ?>
		</a>
	</th>
        <th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=macroproject_end_date" class="hdr"> <?php echo $AppUI->_('Due Date'); ?>
		</a>
	</th>
        <th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=macroproject_actual_end_date" class="hdr"> <?php echo $AppUI->_('Actual'); ?>
		</a>
	</th>
        <th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=task_log_problem" class="hdr"> <?php echo $AppUI->_('P'); ?>
		</a>
	</th>
	<th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=user_username" class="hdr"> <?php echo $AppUI->_('Owner'); ?>
		</a>
	</th>
	<th nowrap="nowrap">
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=total_tasks" class="hdr"> <?php echo $AppUI->_('Tasks'); ?>
		</a>
		<a href="<?php echo $headerGet; ?>&amp;macroorderby=my_tasks" class="hdr"> (<?php echo $AppUI->_('My'); ?>)
		</a>
	</th>
	<th nowrap="nowrap">
...
$CT = "\n\t";
$none = true;
foreach ($macroprojects as $row) { if (!(getPermission('macroprojects', 'view', $row['macroproject_id']))) { continue; }
	...
	if ($macroproFilter == -1 || $row['macroproject_status'] == $macroproFilter 
		...
		$none = false;
		$start_date = ((intval(@$row['macroproject_start_date'])) ? new CDate($row['macroproject_start_date']) : null);
		$end_date = ((intval(@$row['macroproject_end_date']))  ? new CDate($row['macroproject_end_date']) : null);
		$actual_end_date = ((intval(@$row['macroproject_actual_end_date'])) ? new CDate($row['macroproject_actual_end_date']) : null);
...
?>', CAPTION, '<?php echo $AppUI->_('Description'); ?>', CENTER);" onmouseout="nd();">
	...
	<td align="center" nowrap="nowrap"><?php echo htmlspecialchars($row['user_username'], ENT_QUOTES); ?>
	</td>
	<td align="center" nowrap="nowrap"><?php echo htmlspecialchars($row['total_tasks'] . ($row['my_tasks'] ? ' ('.$row['my_tasks'].')' : '')); ?>
	</td>
	<td align="center" nowrap="nowrap"><?php echo $AppUI->_($mpstatus[$row['macroproject_status']]); ?>
	</td>
	</tr>
	<?php 
	}
