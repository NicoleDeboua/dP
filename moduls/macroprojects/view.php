...
if (!($canRead)) {	$AppUI->redirect('m=public&a=access_denied'); } //Check if the proect is viewable.
if (isset($_GET['tab'])) {	$AppUI->setState('ProjVwTab', $_GET['tab']);} //retrieve any state parameters
...
$criticalTasks = ($macroproject_id > 0) ? $obj->getCriticalTasks($macroproject_id) : NULL; //get critical tasks (criteria: task_end_date)
...
$obj = null;
if (!db_loadObject($sql, $obj)) { $AppUI->setMsg('macroproject'); $AppUI->setMsg('invalidID', UI_MSG_ERROR, true); $AppUI->redirect();} 
   else { $AppUI->savePlace(); }

//worked hours now milestones are summed up, too, for consistence with the tasks duration sum
...
if ($hasTasks) {
    ...
    //total hours - same milestone comment as above, also applies to dynamic tasks
    $q->addTable('tasks');
...
else { $worked_hours = $total_hours = $total_macroproject_hours = 0.00; } //no tasks in macroproject so "fake" macroproject data
...
$titleBlock = new CTitleBlock('View macroproject', 'macroprview.gif', $m, "$m.$a"); //setup the title block

//patch 2.12.04 text to search entry box
if (isset($_POST['searchtext'])) { $AppUI->setState('searchtext', $_POST['searchtext']);}
...
if ($canEdit) {
	...
	$titleBlock->addCell(('<input type="submit" class="button-add" value="' . $AppUI->_('new event') 
	                      . '" />'), '', ('<form action="?m=calendar&amp;a=addedit&amp;event_macroproject=' 
	                                    . $macroproject_id . '" method="post">'), '</form>');

	$titleBlock->addCell(('<input type="submit" class="button-add" value="' . $AppUI->_('new file') 
	                      . '" />'), '', ('<form action="?m=files&amp;a=addedit&amp;macroproject_id=' 
	                                    . $macroproject_id . '" method="post">'), '</form>');
}

//$titleBlock->addCell('?m=macroprojects', 'Macroprojects');
//$titleBlock->addCell('', $obj->macroproject_name, 'applet3-48.png');
/*if ($canEdit) {
	$titleBlock->addCell(('?m=macroprojects&amp;a=addedit&amp;macroproject_id=' . $macroproject_id), 'edit this macroproject');
	if ($canDelete) { $titleBlock->addCell('delete macroproject', $canDelete, $msg);	}
	$titleBlock->addCell('?m=tasks&amp;a=organize&amp;macroproject_id=' . $macroproject_id, 'organize tasks');
}
$titleBlock->addCell('?m=macroprojects&amp;a=reports&amp;macroproject_id=' . $macroproject_id, 'reports');
*/
$titleBlock->show();
...
<script type="text/javascript" language="javascript">
	...
	function delIt() {
		if (confirm("<?php 
			echo ($AppUI->_('doDelete', UI_OUTPUT_JS) 
					. ' ' 
					. $AppUI->_('macroproject', UI_OUTPUT_JS) 
	...
	function doDetails() { 
		...
		if (obj.style.display=="none") { obj.style.display="block"; col.src="images/icons/collapse.gif"; } 
			else { obj.style.display="none"; col.src="images/icons/expand.gif"; }
	} 
	<?php //} ?>
</script>
...
<form name="frmDelete" action="./index.php?m=macroprojects" method="post">
	...
	<table class="std" border="0" cellpadding="4" cellspacing="0" width="98%">
		<tr>
			<td style="border: outset #d1d1cd 1px;background-color:#<?php 
				echo $obj->macroproject_color_identifier; ?>" colspan="2">
					<a href="?m=macroprojects&amp;a=addedit&amp;macroproject_id=<?php echo $macroproject_id; ?>"><img src="images/icons/pencil.gif" border="0" width="14" height="14"/></a><a href="javascript:doDetails()">
						<img border="0" align="middle" id="displayImg" alt="" src="images/icons/expand.gif" />
					</a>
				<?php
				echo ('<span style="color:' 
					. bestColor($obj->macroproject_color_identifier) 
					. '; font-weight:bold">' 
			...
			<tr id="contentTr" style="display:none;">
				<td width="50%" valign="top">
					...
					<table cellspacing="1" cellpadding="2" border="0" width="98%">
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Company'); ?>:
							</td>
							<td class="hilite" width="100%"><?php 
								echo (((getPermission('companies', 'view', $obj->macroproject_company)) 
										? ('<a href="?m=companies&amp;a=view&amp;company_id=' 
										. $obj->macroproject_company . '">') : '') 
						...
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Internal Company'); ?>:
							</td>
							<td class="hilite" width="100%"><?php 
								echo (((getPermission('companies', 'view', $obj->macroproject_company_internal)) 
									? ('<a href="?m=companies&amp;a=view&amp;company_id=' 
									. $obj->macroproject_company_internal 
									. '">') : '') 
									. htmlspecialchars($obj->company_name_internal, ENT_QUOTES) 
									. ((getPermission('companies', 'view', $obj->macroproject_company_internal)) ? '</a>' : '')); 
								?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Short Name'); ?>:
							</td>
							<td class="hilite"><?php echo htmlspecialchars(@$obj->macroproject_short_name, ENT_QUOTES); ?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Start Date'); ?>:
							</td>
							<td class="hilite"><?php echo $start_date ? $start_date->format($df) : '-'; ?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Target End Date'); ?>:
							</td>
							<td class="hilite"><?php echo $end_date ? $end_date->format($df) : '-'; ?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Actual End Date'); ?>:
							</td>
							<td class="hilite">
								<?php 
									if ($macroproject_id > 0) {
										echo (($actual_end_date) 
											...
											. $criticalTasks[0]['task_id'] 
											. '">' 
											. '<span '
											. $style.'>'
											.$actual_end_date->format($df)
											.'</span></a>') : '-');
									} 
										else { echo $AppUI->_('Dynamically calculated'); } 
								?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('Target Budget'); ?>:
							</td>
							<td class="hilite"><?php echo $dPconfig['currency_symbol'] ?><?php 
								echo @$obj->macroproject_target_budget; ?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('MacroProject Owner'); ?>:
							</td>
							<td class="hilite"><?php echo $obj->user_name; ?>
							</td>
						</tr>
						<tr>
							<td align="right" nowrap><?php echo $AppUI->_('URL'); ?>:
							</td>
							<td class="hilite"><a href="<?php echo @$obj->macroproject_url; ?>" target="_new"><?php 
									echo @$obj->macroproject_url; ?></a>
							</td>
	</tr>
	<tr>
		<td align="right" nowrap><?php echo $AppUI->_('Staging URL'); ?>:
		</td>
		<td class="hilite"><a href="<?php echo @$obj->macroproject_demo_url; ?>" target="_new"><?php 
				echo @$obj->macroproject_demo_url; ?></a>
		</td>
	</tr>
	<tr>
		<td colspan="2">
	...
	<td width="50%" rowspan="9" valign="top">
		...
		<tr>
			...
			<td class="hilite" width="100%"><?php 
				echo $AppUI->_($mpstatus[$obj->macroproject_status]); ?>
			</td>
		</tr>
		<tr>
			<td align="right" nowrap><?php echo $AppUI->_('Priority'); ?>:
			</td>
			<td class="hilite" width="100%" style="background-color:<?php 
					echo $macroprojectPriorityColor[$obj->macroproject_priority]?>"><?php 
					echo $AppUI->_($macroprojectPriority[$obj->macroproject_priority]); ?>
			</td>
		</tr>
		<tr>
			<td align="right" nowrap><?php echo $AppUI->_('Type'); ?>:
			</td>
			<td class="hilite" width="100%"><?php 
					echo $AppUI->_($mptype[$obj->macroproject_type]); ?>
			</td>
		</tr>
		<tr>
			<td align="right" nowrap><?php echo $AppUI->_('Progress'); ?>:
			</td>
			<td class="hilite" width="100%"><?php 
					printf('%.1f%%', $obj->macroproject_percent_complete); ?>
			</td>
		</tr>
		<tr>
