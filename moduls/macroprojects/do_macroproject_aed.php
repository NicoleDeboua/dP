...
if (!$obj->bind($_POST)) { $AppUI->setMsg($obj->getError(), UI_MSG_ERROR); $AppUI->redirect();}
...
if ($del) {
	...
	if (!$canDelete) {	$AppUI->setMsg($msg, UI_MSG_ERROR);	$AppUI->redirect();	}
	if (($msg = $obj->delete())) {	$AppUI->setMsg($msg, UI_MSG_ERROR);	$AppUI->redirect();	} 
	 else {	$AppUI->setMsg("MacroProject deleted", UI_MSG_ALERT); $AppUI->redirect("m=macroprojects");	}
} 
else {	if (($msg = $obj->store())) { $AppUI->setMsg($msg, UI_MSG_ERROR);} 
	else { 	$isNotNew = @$_POST['macroproject_id'];
...
*/
	...
	if($isNotNew) {	$AppUI->redirect();	}
	 else {	$AppUI->redirect("m=macroprojects&a=addproject&macroproject_id=".$obj->macroproject_id);}
}
?>