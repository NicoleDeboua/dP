...
$row = new CMacroProject();

if (!$row->load($macroproject_id, false) && $macroproject_id > 0) {
	$AppUI->setMsg('MacroProject');
	$AppUI->setMsg("invalidID", UI_MSG_ERROR, true);
	$AppUI->redirect();
} else if (count($companies) < 2 && $macroproject_id == 0) {
	$AppUI->setMsg("noCompanies", UI_MSG_ERROR, true);
	$AppUI->redirect();
}

if ($macroproject_id == 0 && $company_id > 0) {	$row->macroproject_company = $company_id;}

// add in the existing company if for some reason it is dis-allowed
if ($macroproject_id && !array_key_exists($row->macroproject_company, $companies)) {
	$q  = new DBQuery;
	$q->addTable('companies', 'co');
	$q->addQuery('company_name');
	$q->addWhere('co.company_id = '.$row->macroproject_company);
	$sql = $q->prepare();
	$q->clear();
	$companies[$row->macroproject_company] = db_loadResult($sql);
}

// get critical tasks (criteria: task_end_date)
$criticalTasks = ($macroproject_id > 0) ? $row->getCriticalTasks() : NULL;
...
$start_date = new CDate($row->macroproject_start_date);
...
//$ttl = $macroproject_id > 0 ? "Edit MacroProject" : "New MacroProject";
//$titleBlock->addBreadcrumb('?m=macroprojects', 'Macroprojects');
$titleBlock = new CTitleBlock('new macroproject', 'newmacroproject.gif', $m, "$m.$a");
if ($macroproject_id != 0){
	$titleBlock->addCell("?m=macroprojects&amp;a=view&amp;macroproject_id=$macroproject_id", $row->macroproject_name, 'applet3-48.png');
	$titleBlock->addCell('', 'Edition', 'icons/pencil.gif');
}else $titleBlock->addCell('', 'New MacroProject');
$titleBlock->show();

//Build display list for departments
$company_id = $row->macroproject_company;
...
if ($department_selection_list!='' || $macroproject_id) {
	...
	$department_selection_list = ('<input type="button" class="button" value="'
	                              ...
	                              . '<input type="hidden" name="macroproject_departments"');
}
...
if ($macroproject_id == 0 && $contact_id > 0) {
...
if (color) { f.macroproject_color_identifier.value = color; }
//test.style.background = f.macroproject_color_identifier.value;
document.getElementById('test').style.background = '#' + f.macroproject_color_identifier.value; 		//fix for mozilla: does this work with ie? opera ok.
}

function setShort() {
var f = document.editFrm;
var x = 10;
if (f.macroproject_name.value.length < 11) {
	x = f.macroproject_name.value.length;
}
if (f.macroproject_short_name.value.length == 0) {
	f.macroproject_short_name.value = f.macroproject_name.value.substr(0,x);
}
}
...
calendarField = field;
idate = eval('document.editFrm.macroproject_' + field + '.value');
...
function setCalendar(idate, fdate) {
	fld_date = eval('document.editFrm.macroproject_' + calendarField);
	...
	if (calendarField == 'start_date') {
		if (document.editFrm.end_date.value < idate) {
			document.editFrm.macroproject_end_date.value = idate;
...
function submitIt() {
	...
	*/
	$requiredFields = dPgetSysVal('MacroProjectRequiredFields');
...
function setContacts(contact_id_string) {
	...
	document.editFrm.macroproject_contacts.value = contact_id_string;
...
//this bug does NOT occur in Internet explorer
	...
	var url = '?m=public&a=selector&dialog=1&callback=setDepartment&table=departments&company_id='
            + f.macroproject_company.options[f.macroproject_company.selectedIndex].value
            + '&dept_id='
...
function setDepartment(department_id_string) {
	...
	document.editFrm.macroproject_departments.value = department_id_string;
...
<form name="editFrm" action="?m=macroprojects" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" name="macroproject_creator" value="<?php echo $AppUI->user_id;?>" />
	<input name='macroproject_contacts' type='hidden' value="<?php echo implode(',', $selected_contacts); ?>" />
<table cellspacing="0" cellpadding="4" border="0" width="100%" class="std">
<tr>
	<td>
		<input class="button" type="button" name="cancel2" value="<?php echo $AppUI->_('cancel');?>" onclick="javascript:if (confirm('Are you sure you want to cancel.')) {location.href = '?m=macroprojects';}" />
	</td>
	<td align="right">
...
<tr>
	<td width="50%" valign="top">
		...
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('MacroProject Name');?></td>
			<td width="100%" colspan="2">
				<input type="text" name="macroproject_name" value="<?php echo dPformSafe($row->macroproject_name);?>" size="25" maxlength="50" onblur="javascript:setShort();" class="text" /> *
			</td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('MacroProject Owner');?></td>
			<td colspan="2">
<?php echo arraySelect($users, 'macroproject_owner', 'size="1" style="width:200px;" class="text"', $row->macroproject_owner? $row->macroproject_owner : $AppUI->user_id) ?>
			</td>
		</tr>
		<tr>
...
<?php
		echo arraySelect($companies, 'macroproject_company', 'class="text" size="1"', $row->macroproject_company);
?> *</td>
		</tr>
		<tr>
			...
			<td width="100%" nowrap="nowrap" colspan="2">
<?php
		echo arraySelect($companies_internal, 'macroproject_company_internal', 'class="text" size="1"', $row->macroproject_company_internal);
?></td>
 		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('Start Date');?></td>
			<td nowrap="nowrap">	 <input type="hidden" name="macroproject_start_date" value="<?php echo $start_date->format(FMT_TIMESTAMP_DATE);?>" />
		...
		<tr>
			...
			<td nowrap="nowrap">	<input type="hidden" name="macroproject_end_date" value="<?php echo $end_date ? $end_date->format(FMT_TIMESTAMP_DATE) : '';?>" />
		...
		<tr>
			...
			<td>
				<input type="text" name="macroproject_target_budget" value="<?php echo @$row->macroproject_target_budget;?>" maxlength="10" class="text" />
			</td>
		</tr>
		<tr>
...
<tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="macroproject_actual_budget" value="<?php echo @$row->macroproject_actual_budget;?>" size="10" maxlength="10" class="text" />
			</td>
		</tr>
		<tr>
			<td colspan="3"><hr noshade="noshade" size="1" /></td>
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="macroproject_url" value='<?php echo @$row->macroproject_url;?>' size="40" maxlength="255" class="text" />
			</td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('Staging URL');?></td>
			<td colspan="2">
				<input type="text" name="macroproject_demo_url" value='<?php echo @$row->macroproject_demo_url;?>' size="40" maxlength="255" class="text" />
			</td>
		</tr>
		<tr>
			...
			<?php
				...
				//$custom_fields = New CustomFields($m, $a, $row->macroproject_id, "edit");
	...
	<td width="50%" valign="top">
		...
		<tr>
			...
			<td nowrap="nowrap">
				<?php echo arraySelect($macroprojectPriority, 'macroproject_priority', 'size="1" class="text"', $row->macroproject_priority, true);?> *
			</td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('Short Name');?></td>
			<td colspan="3">
				<input type="text" name="macroproject_short_name" value="<?php echo dPformSafe(@$row->macroproject_short_name) ;?>" size="10" maxlength="10" class="text" /> *
			</td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('Color Identifier');?></td>
			<td nowrap="nowrap">
				<input type="text" name="macroproject_color_identifier" value="<?php echo (@$row->macroproject_color_identifier) ? @$row->macroproject_color_identifier : 'FFFFFF';?>" size="10" maxlength="6" onblur="javascript:setColor();" class="text" /> *
			</td>
			...
			<td nowrap="nowrap">
				<span id="test" title="test" style="background:#<?php echo (@$row->macroproject_color_identifier) ? @$row->macroproject_color_identifier : 'FFFFFF';?>;"><a href="#" onclick="javascript:newwin=window.open('?m=public&a=color_selector&dialog=1&callback=setColor', 'calwin', 'width=320, height=300, scrollbars=no');"><img src="./images/shim.gif" border="1" width="40" height="20" alt="" /></a></span>
			</td>
		</tr>
		<tr>
			<td align="right" nowrap="nowrap"><?php echo $AppUI->_('MacroProject Type');?></td>
			<td colspan="3">
				<?php echo arraySelect($mptype, 'macroproject_type', 'size="1" class="text"', $row->macroproject_type, true);?> *
			</td>
		</tr>
		<tr>
			<td colspan="4">
				...
				<tr>
					<td>
						<?php echo arraySelect($mpstatus, 'macroproject_status', 'size="1" class="text"', $row->macroproject_status, true); ?>
					</td>
					<td>
						<strong><?php echo sprintf("%.1f%%", @$row->macroproject_percent_complete);?></strong>
					</td>
				</tr>
				</table>
			</td>
...
< ?php //echo projectSelectWithOptGroup($AppUI->user_id, 'import_tasks_from', 'size="1" class="text"', false, $project_id); ?>
		...
		<tr>
			...
			<td colspan="3">
				<input type="checkbox" name="scale_macroproject" id="scale_macroproject" value="1" />
			</td>
		</tr>-->
		<tr>
			<td colspan="4">
				<?php echo $AppUI->_('Description');?><br />
				<textarea name="macroproject_description" cols="50" rows="10" style="wrap:virtual;" class="textarea"><?php echo dPformSafe(@$row->macroproject_description);?></textarea>
			</td>
		</tr>
		</table>
	</td>
...
<tr>
	<td>
		<input class="button" type="button" name="cancel" value="<?php echo $AppUI->_('cancel');?>" onclick="javascript:if (confirm('Are you sure you want to cancel.')) {location.href = '?m=macroprojects';}" />
	</td>
	<td align="right">
