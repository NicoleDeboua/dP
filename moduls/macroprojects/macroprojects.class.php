...
class CMacroProject extends CDpObject {
	...
	function __construct () { 	parent::__construct ('macroprojects', 'macroproject_id');}
	...
	function check() {
		...
		if (empty($this->macroproject_end_date)) {	$this->macroproject_end_date = null; }
	...
	function canDelete(&$msg, $oid=null) {
		...
		// very anoying having to delete all tasks before being able to delete a project.
	...
	function delete() {
		...
		addHistory('macroprojects', $this->macroproject_id, 'delete', $this->macroproject_name, $this->macroproject_id);
		$q = new DBQuery;
...
function macroprojects_list_data($user_id=false) {
	...
	if ($macroprojectTypeId > -1) {	$q->addWhere('mp.macroproject_type = ' . $macroprojectTypeId);	}
	
	if ($user_id && $addMacroProjectsWithAssignedTasks) {
		$q->addWhere('(tu.user_id = ' . $user_id . ' OR mp.macroproject_owner = ' . $user_id . ')');
	} else if ($user_id) {	$q->addWhere('mp.macroproject_owner = ' . $user_id);}
	
	if ($owner > 0) {	$q->addWhere('mp.macroproject_owner = ' . $owner);	}
...
function makeWhereClauseEachProjectOfAMacroProject($macroproject_id, $whereClause)
{
	...
	if(count($projectsofmacro) > 0)
	{
		...
		foreach($projectsofmacro as $projectReed)
		{
			...
			else	{	$finalWhere .= "($whereClause $projectPrint)";		}
		}
	}
	else{	$finalWhere = ($whereClause.' 10');	}
	return $finalWhere;
}
