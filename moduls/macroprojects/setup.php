<?php /* MACRO_PROJECTS setup.php, v.1.0.0 2012-05-02 */
...
class CSetupMacroProjects {
	...
	function remove() {
		...
		$q->exec();
		$q->clear();
		foreach ($bulk_sql as $s) {	db_exec($s); if (db_error())	$success = 0;	}
	...
	function install() { $dbprefix = dPgetConfig('dbprefix', '');
		...
		$bulk_sql[] = " CREATE TABLE IF NOT EXISTS `{$dbprefix}macroprojects` (
				...
				) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci AUTO_INCREMENT=7 ;";

		$bulk_sql[1] = " CREATE TABLE IF NOT EXISTS `{$dbprefix}macroproject_contacts` (
				  `macroproject_id` int(10) NOT NULL,
				  `contact_id` int(10) NOT NULL
				) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
				
		$bulk_sql[2] = " CREATE TABLE IF NOT EXISTS `{$dbprefix}macroproject_departments` (
				  `macroproject_id` int(10) NOT NULL,
				  `department_id` int(10) NOT NULL
				) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
				
		$bulk_sql[3] = " CREATE TABLE IF NOT EXISTS `{$dbprefix}macroproject_project` (
				  `macroproject_id` int(10) NOT NULL,
				  `project_id` int(10) NOT NULL
				) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
				
		$bulk_sql[4] = " CREATE TABLE IF NOT EXISTS `{$dbprefix}macroproject_macroproject` (
		  `macroproject_father` int(10) NOT NULL,
		  `macroproject_son` int(10) NOT NULL
		) ENGINE=MyISAM DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;";
		
		$bulk_sql[5] = " INSERT INTO `sysvals` (`sysval_id`, `sysval_key_id`, `sysval_title`, `sysval_value`) VALUES
        ...
        $bulk_sql[6] = " ALTER TABLE `forums` ADD `forum_macroproject` INT( 11 ) NOT NULL DEFAULT '0' AFTER `forum_project`;";
