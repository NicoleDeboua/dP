...
require_once($AppUI->getModuleClass('companies')); // load the companies class to retrieved denied companies
/* MACRO_PROJECTS departments_tab.viewuser.macroprojects_gantt.php, v 0.1.0 2012/05/30 */
...
if (isset($_GET['tab'])) {	$AppUI->setState('DeptMacroProjIdxTab', $_GET['tab']);}
if (isset($_POST['show_form'])) { $AppUI->setState('addProjWithOwnerInDep',  dPgetParam($_POST, 'add_pwoid', 0)); }
$addPwOiD = $AppUI->getState('addProjWithOwnerInDep') ? $AppUI->getState('addProjWithOwnerInDep') : 0;
$extraGet = '&user_id='.$user_id;
?>
<table class="tbl" width="100%" border="0" cellpadding="3" cellspacing="1" >
	<tr>
...
<?php $min_view = true; require(DP_BASE_DIR.'/modules/macroprojects/vw_gantt.php'); ?>
