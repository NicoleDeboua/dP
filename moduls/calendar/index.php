...
if (isset($_REQUEST['company_id'])) { $AppUI->setState('CalIdxCompany', intval($_REQUEST['company_id']));}
...
$titleBlock = new CTitleBlock('Monthly Calendar', 'logo_calendar.gif', $m, "$m.$a");
$titleBlock->addCell($AppUI->_('Company').':');
$titleBlock->addCell(arraySelect($companies, 'company_id', 'onchange="javascript:document.pickCompany.submit()" class="text"', 
								 $company_id), '', ('<form action="' . $_SERVER['REQUEST_URI'] . '" method="post" name="pickCompany">'), '</form>');
$titleBlock->addCell($AppUI->_('Event Filter') . ':');
$titleBlock->addCell(arraySelect($event_filter_list, 'event_filter', 'onchange="javascript:document.pickFilter.submit()" class="text"',
...
<table cellspacing="0" cellpadding="0" border="0" width="100%">
	...
	$date = new CDate($date); // establish the focus 'date'

	$first_time = new CDate($date); // prepare time period for 'events'
	$first_time->setDay(1);
	...
	getTaskLinks($first_time, $last_time, $links, 20, $company_id); // assemble the links for the tasks
	getEventLinks($first_time, $last_time, $links, 20); // assemble the links for the events

	$cal = new CMonthCalendar($date); // create the main calendar
