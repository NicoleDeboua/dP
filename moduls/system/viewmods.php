<?php /* SYSTEM $Id: viewmods.php 6046 2010-10-04 ajdonnison $ */
...
$q->addQuery('*');
$q->addTable('modules');
foreach ($hidden_modules as $no_show) {	$q->addWhere('mod_directory != \'' . $no_show . '\''); }
$q->addOrder('mod_ui_order');
$modules = db_loadList($q->prepare());
// get the modules actually installed on the file system
$modFiles = $AppUI->readDirs('modules');

$titleBlock = new CTitleBlock('Modules', 'logo_viewmods.gif', $m, $m . "." . $a);
$titleBlock->addCrumb('?m=system', 'System Admin');
$titleBlock->show();
?>

<table class="tbl" width="80%" border="0" cellpadding="2" cellspacing="1" align="center" >
<tr>
...
foreach ($modules as $row) {
	...
	if (isset($modFiles[$row['mod_directory']])) { $modFiles[$row['mod_directory']] = ''; }
	...
	$s .= '<td align="center">';
	$s .= ('<img src="./images/icons/updown.gif" width="10" height="15" border="0" usemap="#arrow' . $row['mod_id'] . '" alt="" />');
	if ($canEdit) {
		$s .= '<map id="arrow'.$row['mod_id'].'" name="arrow'.$row['mod_id'].'">'."\n";
		if ($row['mod_ui_order'] > 0) { $s .= '<area coords="0,0,10,7" href="' . $query_string . '&amp;cmd=moveup" alt="" />'."\n";	}
