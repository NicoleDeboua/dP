...
$dPcfg = new CConfig();
$rs = $dPcfg->loadAll('config_group'); // retrieve the system configuration data
if (isset($_GET['tab'])) { $AppUI->setState('ConfigIdxTab', $_GET['tab']); } // retrieve any state parameters
$tab = $AppUI->getState('ConfigIdxTab') !== NULL ? $AppUI->getState('ConfigIdxTab') : 0;
$active = intval(!$AppUI->getState('ConfigIdxTab'));

$titleBlock = new CTitleBlock('System Configuration','control-center.gif', $m);
...
$output  = null;
$last_group = '';
foreach ($rs as $c) { $tooltip = 'title="'.$AppUI->_($c['config_name'].'_tooltip').'"';
	...
	$output .= '<tr>
					<td class="item" width="300">'.$AppUI->_($c['config_name'].'_title').'
					</td>
					<td align="left">' 	. $entry ."\n\t" 
										. '<a href="#" onclick="javascript:window.open(' 
										. '\'?m=system&amp;a=systemconfig_help&amp;dialog=1&amp;cn='
										. $c['config_name']. "', 'contexthelp', "
										. "'width=300, height=200, left=50, top=50, scrollbars=yes, resizable=yes')\" "
										. $tooltip.'>(?)</a>
...
<table class="tbl" align="center" border="0" cellpadding="3" cellspacing="2" width="600" >
	<tr>
		<th align="center" colspan="2">
			<?php echo $AppUI->_('System config');?>
		</th>

	</tr>
	<tr>
		<td align="left" colspan="2">
			<?php echo $AppUI->_('syscfg_intro');?>
		</td>
	</tr>
